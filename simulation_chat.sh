...
# Fonction cleanup (Docker gère le serveur, on nettoie juste l'audio)
cleanup() {
    echo -e "\n${BLUE}$(L stopping)${NC}"
...
# Vérifier que Docker est connecté (ne démarre pas le serveur, Docker le fait)
check_server() {
    echo -e "${BLUE}$(L connecting)${NC}"

    # Vérifier que le serveur Docker est prêt
    for i in {1..10}; do
    ...
    echo ""
    echo -e "${YELLOW}Start the services first with:${NC}"
...
chat() {
    ...
    while true; do
        ...
        if [ "$message" = "/setup" ]; then
            echo ""
            echo -e "${YELLOW}Configuration via Docker${NC}"
            echo -e "${GRAY}Modifie ton fichier .env puis relance: docker compose up -d${NC}"
            echo ""
            continue
        fi
...
check_deps       # Check dependencies (jq, sox optional)
check_server     # Verify Docker API is running
select_user
chat
